...
class PivxTestFramework():
    ...
    def get_prevouts(self, node_id, utxo_list, nHeight=-1):
        ...
        :return: prevouts:         ({bytes --> (int, bytes, int)} dictionary)
                                   maps CStake "uniqueness" (i.e. serialized COutPoint -or hash stake, for zModden-)
                                   to (amount, prevScript, timeBlockFrom).
                                   For zModden prevScript is replaced with serialHash hex string.
        """
    ...
    def stake_block(self, node_id,
        ...
        for tx in vtx:
            if not fDoubleSpend:
                # assume txes don't double spend zModden inputs when fDoubleSpend is false. It needs to
